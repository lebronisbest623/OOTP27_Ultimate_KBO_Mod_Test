...
#include "amateur_player_quality.h"
...
/* Migrated from amateur_player_quality_state.inc. */
...
/* Migrated from amateur_reputation_seed.inc. */
...
/* Migrated from amateur_reputation_history.inc. */
...
/* Migrated from amateur_reputation_memory.inc. */
...
/* Migrated from amateur_reputation_update.inc. */
static int kbo_compare_amateur_reputation_update_rows(const void* a, const void* b)
{
...
static uint8_t kbo_amateur_reputation_clamp_for_league(uint32_t league_id, int32_t value)
{
...
static int kbo_append_amateur_reputation_history(
    uint32_t league_id,
...
static int kbo_update_amateur_reputation_for_league(uint32_t league_id, const char* source, uint32_t year)
{
...
/* Migrated from amateur_assignment_processed.inc. */
...
/* Migrated from amateur_assignment_policy.inc. */
...
/* Migrated from amateur_assignment_debug.inc. */
static int kbo_amateur_assignment_debug_csv_empty(HANDLE file)
{
    LARGE_INTEGER size;
...
static void kbo_amateur_assignment_debug_write_text(HANDLE file, const char* text)
{
...
static void kbo_amateur_assignment_debug_write_csv_text(HANDLE file, const char* text)
{
    if (file == INVALID_HANDLE_VALUE) {
        return;
    }
...
static void kbo_amateur_assignment_append_debug_csv(
    const char* phase,
    const char* source,
    uint32_t player_id,
    uint32_t league_id,
...
/* Migrated from amateur_assignment_candidates.inc. */
static uint32_t kbo_amateur_assignment_candidate_weight(
    uint32_t league_id,
    int32_t quality_score,
    int assignment_tier,
    int32_t target,
...
static int kbo_amateur_assignment_collect_candidates(
    uint32_t league_id,
...
static void kbo_amateur_assignment_refresh_player_counts(
    uint32_t league_id,
    KboAmateurAssignmentCandidate* candidates,
    int candidate_count)
{
...
static void kbo_amateur_assignment_note_player_count_delta(uint32_t league_id, uint32_t team_id, uint8_t* player, int32_t delta)
{
...
static int kbo_amateur_assignment_get_cached_candidates(
    uint32_t league_id,
...
static int kbo_amateur_assignment_find_candidate_info(
    uint32_t league_id,
    uint32_t team_id,
...
/* Migrated from amateur_assignment_selection.inc. */
static uint8_t* kbo_choose_amateur_assignment_team(
    uint8_t* player,
    uint32_t league_id,
...
/* Migrated from amateur_assignment_reroute.inc. */
...
/* Migrated from amateur_assignment_result.inc. */
