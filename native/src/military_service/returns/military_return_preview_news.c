...
static int kbo_military_return_preview_marker_exists(const char* key)
{
    if (key == NULL || key[0] == '\0') {
        return 1;
    }
...
static void kbo_military_return_preview_persist_marker(const char* key, const char* source)
{
...
static int kbo_military_return_preview_collect(
...
static void kbo_military_return_preview_player_name(
...
static int kbo_military_return_preview_render_body(
