...
$NativePath = Join-Path $RepoRoot "native\src\build_verify\supported_builds.generated.inc"
...
foreach ($Build in $Builds) {
    $NativeLines += "#define KBO_SUPPORTED_OOTP_BUILD_$($Build.CId)_TIMESTAMP 0x$('{0:X8}' -f $Build.Timestamp)u"
    $NativeLines += "#define KBO_SUPPORTED_OOTP_BUILD_$($Build.CId)_SIZE_OF_IMAGE 0x$('{0:X8}' -f $Build.SizeOfImage)u"
}
$NativeLines += @(
    "",
...
foreach ($Build in $Builds) {
    $NativeLines += '    {{0x{0:X8}u, 0x{1:X8}u, "{2}"}},' -f $Build.Timestamp, $Build.SizeOfImage, ($Build.Label -replace '"', '\"')
}
$NativeLines += "};"

Set-Content -Encoding UTF8 -Path $CSharpPath -Value ($CsLines -join "`r`n")
Set-Content -Encoding UTF8 -Path $NativePath -Value ($NativeLines -join "`r`n")

Write-Host "Generated $CSharpPath"
Write-Host "Generated $NativePath"
